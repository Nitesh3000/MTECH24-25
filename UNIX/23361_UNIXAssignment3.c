...
void entryforlog(const char *content, const char *action);
...
void reserveSpaceForTelephoneBook(const char *filename, size_t numEntries) {
    ...
    close(fileDescriptor);
    entryforlog(filename, "Create Telephone Book");
}

void entryforlog(const char *content, const char *action) {
    int log;
    if ((log = open(LOGFile, O_WRONLY | O_CREAT | O_APPEND, 0664)) == -1) {
        perror("Error occurred while opening the log file");
        exit(EXIT_FAILURE);
    }
    time_t now;
    struct tm *tm_info;
...
void readTelephoneBookEntry(const char *filename, size_t index) {
    ...
    char content[100];
    sprintf(content, "Index: %zu, Entry: %s", index, entry);
    entryforlog(filename, "Read");
}
...
int main(){
    int log = open(LOGFile, O_WRONLY | O_CREAT | O_APPEND, 0664);
    if(log == -1){
    	perror("Creation error of the LogFile");
    	exit(EXIT_FAILURE);
    }
    close(log);
