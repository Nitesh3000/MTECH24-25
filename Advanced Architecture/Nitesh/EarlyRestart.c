...
void writeToL1Cache(Cache* l1Cache, int address, int value, MainMemory* mainMemory) {
    ...
    if (!l1Cache->lines[line_index].valid || l1Cache->lines[line_index].tag != block_number) {
        printf("\nL1 Cache Miss - Fetching data from Main Memory...\n");

        l1Cache->lines[line_index].valid = 1;
        l1Cache->lines[line_index].tag = block_number;

        int block_start_address = block_number * BLOCK_SIZE;
            printf("searching the required word in its block in main memory\n ");
            printf("Reading word %d from Main Memory to L1 Cache...\n", block_start_address + offset);
                printf("Passing the required word to processor...\n");
        ...
        for (int i = 0; i < BLOCK_SIZE; i++) {
            ...
            if (i == offset) {
                printf("the word %d is already sent to processor..going to the next word in the memory (if present)\n",block_start_address+i);
                
            } else {
                printf("Continuing fetching word %d to the cache...\n", block_start_address + i);
            }
        }
    }
    printf("\n");
...
int main() {
    ...
    while (1) {
        ...
        if (address >= MEMORY_SIZE){
            printf("%d address is not present in the main memory..address is present from [0 - %d] \n ", address,MEMORY_SIZE-1);
        }
        else{

            int data = readFromCache(l1Cache, address);
            if (data == -1) {
                //printf("\nData not found in L1 cache - Fetching from Main Memory...\n");
